  ...
  <div class="contactList">
    <ul v-if="applyData.length != 0">
      <li v-for="(item,index) in applyData">
        ...
        <span class="editbtn" @click="editItem(item)"></span>
      </li>
    </ul>
...
<script>
import InsurClientEdit from './InsClientEdit'
export default {
  props:['passenger'],
  created:function(){
  ...
  data:function(){
    return{
      applyData:this.passenger,
      editStr:{"edit":false,"list":{}}
    }
  },
  methods:{
    checkOne:function(n){
    ...
    editItem:function(v){
      this.editStr.list = v
      this.editStr.edit = true
    },
    editConfirm:function(){
      this.editStr.edit = false
    },
    confirm:function(){
      var scrData = []
