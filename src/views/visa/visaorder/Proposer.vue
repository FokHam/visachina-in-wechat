...
<div class="apply-add" id="apply-add">
  <div class="field">
    <div class="item">
      <span>中文名字</span>
      <div class="ipt">
        <input type="text" v-model="proposerInfo.name" placeholder="与证件保持一致">
      </div>      
    </div>
    <div class="item">
      <span>英文名字</span>
      <div class="ipt">
        <input type="text" v-model="proposerInfo.ename" placeholder="例如：Zhangsan">
      </div>      
    </div>
    <div class="item">
      <span>身份证号</span>
      <div class="ipt" @click="typedis=true">
        <input type="text" v-model="proposerInfo.idtype" readonly placeholder="选择证件类型">
      </div> 
      <div class="idipt">
        <input type="text" v-model="proposerInfo.idnum" placeholder="输入证件号码">
      </div>      
    </div>    
    <div class="item select">
      <span>出生日期</span>
      <div class="ipt" @click="openDatepicker">
        <input type="text" v-model="proposerInfo.birthday" readonly placeholder="与证件保持一致">
      </div> 
    </div>
    <div class="item">
      <span>手机号码</span>
      <div class="ipt">
        <input type="text" v-model="proposerInfo.phone" placeholder="用于接收投保信息">
      </div>      
    </div>
    <div class="item">
      <span>电子邮箱</span>
      <div class="ipt">
        <input type="text" v-model="proposerInfo.email" placeholder="请填写常用邮箱">
      </div>      
    </div>
  </div>
...
export default {
  ...
  methods:{
    ...
    dateConfirm:function(t){
      var m = t.getMonth()+1
      this.proposerInfo.birthday = t.getFullYear() +'-'+ m +'-'+ t.getDate()
    },
    confirm:function(){
